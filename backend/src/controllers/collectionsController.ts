...
export const addPaletteToCollection = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Authentication required' })
    }
...
export const removePaletteFromCollection = async (req: AuthRequest, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Authentication required' })
    }
...
export const addPaletteToCollectionValidation = [
  body('collectionId')
    .notEmpty()
